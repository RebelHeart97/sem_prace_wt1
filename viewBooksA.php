...
.banner{ font-family: mujfont; text-align:center; font-size:40px; color:white; font-weight:bold; border-top:3px solid white; border-bottom:3px solid white; width:100%; text-shadow:1px 1px 2px black;}
...
table{background-color: white; border:3px solid grey; margin:auto; font-family: mujfont; border-spacing: 1px; font-size: 17px; overflow:scroll; margin-top:-15%; margin-left:20%;}
...
<a href="addBook.php">Přidání knihy</a>
...
<div class="hledani">
<form  method="get" action="searchBookA.php"  id="searchform">
...
<form  method="get" action="searchAuthorA.php"  id="searchform">
...
function TlacitkoProRazeni($polozka,$popis)
{
 global $Nazev;
  return
 "<A HREF='viewBooksA.php?orderby=$polozka&Nazev=".
  URLEncode($_GET[Nazev])."'>". "<IMG SRC=arr_up.png WIDTH=20 HEIGHT=20 title=Vzestupně(A-Z,0-9)></A>&nbsp;".$popis."&nbsp;".
"<A HREF='viewBooksA.php?orderby=$polozka+DESC&Nazev=".
...
$oc=$row["id_knihy"];
  ...
  echo  "<A HREF='upravit.php?oc=$oc'>Upravit</A>";
  echo "</td>";
  
  echo "<td align=center>";
    echo  "<A HREF='smazat.php?oc=$oc'>Smazat</A>";   
   echo "</td>";
